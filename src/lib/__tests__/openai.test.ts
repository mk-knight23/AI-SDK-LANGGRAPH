    ...
    it("should return code review result with parsed JSON", async () => {
      ...
      expect(mockCreate).toHaveBeenCalledWith({
        ...
        messages: [
          {
            role: "system",
            content: "You are a senior software engineer doing code reviews.",
          },
          {
            role: "user",
  ...
  describe("explainCode", () => {
    it("should return code explanation as string", async () => {
      const mockExplanation =
        "This code declares a constant variable x with the value 1.";
...
2. Recursive case: return n multiplied by factorial(n-1)
  ...
  describe("API error handling", () => {
    it("should propagate errors from reviewCode", async () => {
      ...
      await expect(reviewCode("code", "javascript")).rejects.toThrow(
    ...
    it("should propagate errors from generateTests", async () => {
      ...
      await expect(generateTests("code", "javascript")).rejects.toThrow(
        "Network error",
      );
    });
    ...
    it("should propagate errors from explainCode", async () => {
      ...
      await expect(explainCode("code", "javascript")).rejects.toThrow(
        "Invalid API key",
      );
    });
  });
